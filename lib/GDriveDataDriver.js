...
class GDriveDataDriver {
	...
	async getData(type, defaultValue = []) {
		...
		try {
			const data = await this.fileDriver.getFile(dataFileName, 'application/json');
	...
	async saveData(type, data) {
		...
		await this.fileDriver.saveFile(dataFileName, encodedData, 'application/json');
		await this.fileDriver.saveFile('backup/' + type + '.' + this.computeFileHash(encodedData) + '.json', encodedData, 'application/json');
	}
